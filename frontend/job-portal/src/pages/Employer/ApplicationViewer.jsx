...
const ApplicationViewer = () => {
  ...
  const handleDownloadResume = (resumeUrl) => {
    ...
    else toast.error("No resume available");
  };

  // Handle status change and trigger backend email notification
  const handleStatusChange = async (applicationId, newStatus) => {
    try {
      ...
      toast.success(
        `Application status updated to "${newStatus}" and applicant notified`
      );
    } catch (err) {
  ...
  <img
    ...
    alt={application.applicant?.name || "Unknown"}
    className="h-12 w-12 rounded-full object-cover"
  />
) : (
  <div className="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center">
    <span className="text-blue-600 font-semibold">
      {getInitials(application?.applicant?.name || "Unknown")}
    </span>
  </div>
)}
                              </div>
